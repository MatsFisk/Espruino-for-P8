...
notify.im = Boolean(require("Storage").read("im.log"))
  ? require("Storage").readJSON("im.log")
  : [];
notify.info = Boolean(require("Storage").read("info.log"))
  ? require("Storage").readJSON("info.log")
  : [];
notify.call = Boolean(require("Storage").read("call.log"))
  ? require("Storage").readJSON("call.log")
  : [];
...
var set = {
    ...
    if (this.gIsB) {
      ...
      if (global["\xFF"].BLE_GATTS) {
        if (global["\xFF"].BLE_GATTS.connected)
          global["\xFF"].BLE_GATTS.disconnect().then(function(c) {
            this.gIsB = 0;
          });
      } else gIsB = 0;
    }
  ...
  updateSettings: function() {
    require("Storage").write("setting.json", set.def);
  },
  resetSettings: function() {
    set.def = {
      name: "p8", //Set the name to be broadcasted by the Bluetooth module.
      timezone: 3, //Timezone
  ...
  clin: 0, //not settable
  upd: function() {
    //run this for settings changes to take effect.
...
*/
      ...
      this.hidM = undefined;
      if (global["\xFF"].modules.ble_hid_controls)
    ...
    if (this.def.atc) eval(require("Storage").read("atc"));
      ...
      NRF.setServices(undefined, {
        uart: this.def.cli || this.def.gb ? true : false,
        hid: this.def.hid && this.hidM ? this.hidM.report : undefined
      });
      if (this.atcW) {
        this.atcW = undefined;
        this.atcR = undefined;
...
function bdis() {
  ...
  if (set.bt == 1)
    handleInfoEvent({ src: "BT", title: "BT", body: "Disconnected" });
  else if (set.bt == 2)
    handleInfoEvent({ src: "BT", title: "IDE", body: "Disconnected" });
  else if (set.bt == 3)
    handleInfoEvent({ src: "BT", title: "GB", body: "Disconnected" });
  else if (set.bt == 4)
    handleInfoEvent({ src: "BT", title: "ATC", body: "Disconnected" });
  else if (set.bt == 5)
    handleInfoEvent({ src: "BT", title: "ESP", body: "Disconnected" });
  set.bt = 0;
  ...
  function(s) {
    ...
    if (face.pageCurr < 0) {
      if (global.w && s.state == 1) {
        ...
        g.drawString(
          w.battVoltage(1) + "%",
          125 - g.stringWidth(w.battVoltage(1) + "%") / 2,
          200
        );
        g.flip();
      ...
      if (data[1] < this.xedge || data[1] >= 220) {
        ...
        if (this.go) {
          if (!w.gfx.isOn && face.appCurr != "") {
            ...
            if (face.appCurr == "main" && face.pageCurr == 2)
              face.go("main", 0);
            else {
...
*/
//end
if (Boolean(require("Storage").read("images")))
  eval(require("Storage").read("images"));
